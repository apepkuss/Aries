...
async fn execute_subtask_with_react(
    ...
    .await;
    inject_user_request_context(&mut messages, user_request);
...
pub(crate) async fn build_context_for_react(
    ...
    let system_prompt = if !active_skills.is_empty() {
        ...
        let is_prompt_only = !has_scripts;
...
"#
        ...
        if is_prompt_only {
            // Prompt-only skill: no tools, no action tags, just follow instructions
            format!(
...
## Instructions
1. This is a **prompt-only skill** — it has NO executable scripts or tools.
2. Follow the skill instructions above to directly process the user's request.
3. Do NOT call any tools. Do NOT use <action> tags. Simply apply the skill instructions and produce the result.
...
- Then immediately provide your result in <final_answer></final_answer> tags
...
/// Inject the original user request into the message list so that subtask
/// executors have access to the full user content (e.g., the text to process).
///
/// Inserts a user message with the original request *before* the last user
/// message (which is the task prompt "Please complete the following task: ...").
/// This preserves the task prompt as the final user message while providing
/// the full context the subtask may need.
fn inject_user_request_context(
    ...
    let context_msg = ChatCompletionRequestMessage::new_user_message(
        ChatCompletionUserMessageContent::Text(format!(
            "## Original User Request\n\n{}",
            user_request
        )),
        None,
    );
    // Insert before the last message (which is typically the task prompt)
    let insert_pos = if messages.len() > 1 {
        messages.len() - 1
    } else {
        messages.len()
    };
...
fn build_subagent_system_prompt(
    ...
    if !active_skills.is_empty() {
        ...
        // For prompt-only skills (no scripts), instruct the Sub-Agent to
        // follow skill instructions directly without calling any tools.
        let all_prompt_only = active_skills.iter().all(|s| s.scripts.is_empty());
        if all_prompt_only {
            prompt.push_str("**IMPORTANT**: This is a prompt-only skill with no executable scripts. Follow the skill instructions above to process the user's request directly. Do NOT call any tools — just apply the instructions and return the result as text.\n\n");
        }
